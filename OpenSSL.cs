...
namespace OpenSSL
{
    public static class OpenSslKey
    {
        ...
        public static string SignString(string pKeyFile, string pPassword, string OriginalString)
        {
            ...
            {
                return "Error: .key file does not exist " + pKeyFile;
            }
            ...
            if (rsa != null)
            {
                ...
                byte[] SignedBytes = rsa.SignData(CO, SHA256.Create());
        ...
        public static string VerifyFirm(string string_toverify, string string_signed, string public_key )  
        {
            try
            {
                byte[] bytes = Convert.FromBase64String(public_key);
                X509Certificate2 cert = new X509Certificate2(bytes);
                RSA rsa1 = cert.PublicKey.GetRSAPublicKey();
                bool value = rsa1.VerifyData(Encoding.UTF8.GetBytes(string_toverify), Convert.FromBase64String(string_signed), HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1 );
                ...
                {
                    return "Error: The string is not signed by the issuer of the public key";
                }
